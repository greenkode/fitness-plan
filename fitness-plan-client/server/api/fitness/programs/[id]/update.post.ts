...
export default defineEventHandler(async (event) => {
  ...
  const userProgramId = getRouterParam(event, 'id')
  if (!userProgramId) throw createError({ statusCode: 400, statusMessage: 'Program ID required' })
  ...
  const existing = await db.query.userPrograms.findFirst({
    where: and(eq(userPrograms.id, userProgramId), eq(userPrograms.userId, user.id)),
  })
  if (!existing) {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      await tx.update(programTemplates)
        .set({
          name: program.name,
      ...
      await tx.delete(workoutAssignments).where(and(
        eq(workoutAssignments.userProgramId, userProgramId),
      ...
      return { templateId, userProgramId }
    })
