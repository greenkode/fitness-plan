...
import com.krachtix.identity.process.domain.model.ProcessBasicInfo
import com.krachtix.identity.process.domain.model.ProcessTransitionInfo
...
interface ProcessRepository : JpaRepository<ProcessEntity, Long> {
    ...
    @Query(
        """
        SELECT new com.krachtix.identity.process.domain.model.ProcessBasicInfo(
    ...
    @Query(
        """
        SELECT new com.krachtix.identity.process.domain.model.ProcessTransitionInfo(
    ...
    @Query(
        ...
        AND p.state = :state
        AND EXISTS (
            SELECT 1 FROM ProcessRequestStakeholder s
            WHERE s.processRequest.process = p
