...
import com.krachtix.identity.core.invitation.dto.InviteUserResult
...
class InviteUserCommandHandler(
    ...
    override fun handle(command: InviteUserCommand): InviteUserResult {
        ...
        log.info { "Creating user invitation process for ${newUser.email?.value}" }
        ...
        val process = processGateway.createProcess(
            CreateNewProcessPayload(
                ...
                description = "User invitation for ${newUser.email?.value} to join ${merchant.clientName}",
        ...
        notificationClient.sendNotification(
            recipients = listOf(MessageRecipient(address = newUser.email?.value ?: "")),
            ...
            parameters = mapOf(
                ...
                "invited_by" to "${invitingUser.firstName ?: ""} ${invitingUser.lastName ?: ""}".trim()
                    .ifEmpty { invitingUser.email?.value ?: "" },
        ...
        log.info { "User invitation sent successfully to ${newUser.email?.value}" }
