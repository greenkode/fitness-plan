    ...
    fun sendTwoFactorCode(user: OAuthUser, code: String, ipAddress: String? = null) {
        ...
        runCatching {
            notificationClient.sendNotification(
                recipients = listOf(MessageRecipient(user.email?.value ?: "", user.fullName())),
        ...
        }.onSuccess { log.info { "2FA code email sent to ${user.email?.value}" } }
            .onFailure { e ->
                log.error(e) { "Failed to send 2FA code email to ${user.email?.value}" }
