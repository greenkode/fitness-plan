...
interface OAuthRegisteredClientRepository : JpaRepository<OAuthRegisteredClient, UUID> {
    ...
    @Query("SELECT DISTINCT c FROM OAuthRegisteredClient c LEFT JOIN FETCH c.settings LEFT JOIN FETCH c.scopes WHERE c.id = :id")
    fun findByIdWithSettingsAndScopes(id: UUID): Optional<OAuthRegisteredClient>
