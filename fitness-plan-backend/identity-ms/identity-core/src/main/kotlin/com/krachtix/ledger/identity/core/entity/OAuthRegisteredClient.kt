...
class OAuthRegisteredClient() : AuditableEntity() {
    ...
    fun getClientSecretForEnvironment(environment: EnvironmentMode): String? {
        return when (environment) {
            EnvironmentMode.SANDBOX -> sandboxClientSecret
            EnvironmentMode.PRODUCTION -> productionClientSecret
        }
    }

    fun getClientSecretExpiryForEnvironment(environment: EnvironmentMode): Instant? {
        return when (environment) {
            EnvironmentMode.SANDBOX -> sandboxClientSecretExpiresAt
            EnvironmentMode.PRODUCTION -> productionClientSecretExpiresAt
        }
    }
    ...
    val clientSecret: String?
        get() = getClientSecretForEnvironment(environmentMode)

    val clientSecretExpiresAt: Instant?
        get() = getClientSecretExpiryForEnvironment(environmentMode)
