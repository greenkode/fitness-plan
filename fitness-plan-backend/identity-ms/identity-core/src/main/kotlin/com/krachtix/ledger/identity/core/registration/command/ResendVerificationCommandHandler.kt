...
class ResendVerificationCommandHandler(
    ...
    override fun handle(command: ResendVerificationCommand): ResendVerificationResult {
        ...
        val user = userRepository.findByEmail(Email(normalizedEmail))
        ...
        val processPayload = CreateNewProcessPayload(
            ...
            type = ProcessType.EMAIL_VERIFICATION,
            description = "Email verification resend for ${user.email?.value}",
    ...
    private fun sendVerificationEmail(user: OAuthUser, token: String) {
        ...
        log.info { "Verification email resent to: ${user.email?.value}" }
    }
}
