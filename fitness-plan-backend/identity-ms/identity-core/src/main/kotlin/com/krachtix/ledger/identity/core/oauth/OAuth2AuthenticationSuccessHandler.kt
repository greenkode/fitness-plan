...
import com.krachtix.identity.commons.audit.AuditPayloadKey
...
class OAuth2AuthenticationSuccessHandler(
    ...
    override fun onAuthenticationSuccess(
        ...
        log.info { "OAuth2 login successful for user: ${user.email?.value} via $provider" }
        ...
        publishAuditEvent(user.id.toString(), user.email?.value ?: "", provider.name, ipAddress)
    ...
    private fun publishAuditEvent(userId: String, email: String, provider: String, ipAddress: String?) {
        applicationEventPublisher.publishEvent(
            com.krachtix.identity.commons.audit.AuditEvent(
                actorId = userId,
                actorName = email,
                merchantId = "unknown",
                identityType = com.krachtix.identity.commons.audit.IdentityType.USER,
                resource = com.krachtix.identity.commons.audit.AuditResource.IDENTITY,
