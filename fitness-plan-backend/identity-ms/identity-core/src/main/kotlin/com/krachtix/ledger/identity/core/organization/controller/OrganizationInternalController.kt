package com.krachtix.identity.core.organization.controller

import com.krachtix.commons.internal.OrganizationResponse
import com.krachtix.commons.internal.UpdateOrganizationDatabaseRequest
import com.krachtix.commons.exception.RecordNotFoundException
import com.krachtix.identity.core.organization.repository.OrganizationRepository
import io.github.oshai.kotlinlogging.KotlinLogging
import org.springframework.http.HttpStatus
import org.springframework.security.access.prepost.PreAuthorize
import org.springframework.transaction.annotation.Transactional
import org.springframework.web.bind.annotation.DeleteMapping
import org.springframework.web.bind.annotation.GetMapping
import org.springframework.web.bind.annotation.PatchMapping
import org.springframework.web.bind.annotation.PathVariable
import org.springframework.web.bind.annotation.PutMapping
import org.springframework.web.bind.annotation.RequestBody
import org.springframework.web.bind.annotation.RequestMapping
import org.springframework.web.bind.annotation.ResponseStatus
import org.springframework.web.bind.annotation.RestController
import java.time.Instant
import java.util.UUID

private val log = KotlinLogging.logger {}

@RestController
@RequestMapping("/internal/organizations")
class OrganizationInternalController(
    private val organizationRepository: OrganizationRepository
) {

    @GetMapping("/{organizationId}")
    @PreAuthorize("hasAuthority('SCOPE_internal')")
    fun getOrganization(@PathVariable organizationId: UUID): OrganizationResponse {
        log.info { "Fetching organization: $organizationId" }

        val organization = organizationRepository.findById(organizationId)
            .orElseThrow { RecordNotFoundException("Organization not found: $organizationId") }

        return OrganizationResponse(
            id = organization.id,
            name = organization.name,
            slug = organization.slug,
            databaseName = organization.databaseName,
            databaseCreated = organization.databaseCreated,
            maxKnowledgeBases = organization.maxKnowledgeBases
        )
    }

    @PatchMapping("/{organizationId}/database")
    @PreAuthorize("hasAuthority('SCOPE_internal')")
    @Transactional
    fun updateOrganizationDatabase(
        @PathVariable organizationId: UUID,
        @RequestBody request: UpdateOrganizationDatabaseRequest
    ): OrganizationResponse {
        log.info { "Updating database for organization: $organizationId -> ${request.databaseName}" }

        val organization = organizationRepository.findById(organizationId)
            .orElseThrow { RecordNotFoundException("Organization not found: $organizationId") }

        organization.databaseName = request.databaseName
        organization.databaseCreated = true
        organization.updatedAt = Instant.now()

        organizationRepository.save(organization)

        log.info { "Organization database updated successfully: $organizationId" }

        return OrganizationResponse(
            id = organization.id,
            name = organization.name,
            slug = organization.slug,
            databaseName = organization.databaseName,
            databaseCreated = organization.databaseCreated,
            maxKnowledgeBases = organization.maxKnowledgeBases
        )
    }

    @GetMapping("/{merchantId}/restriction")
    @PreAuthorize("hasAuthority('SCOPE_internal')")
    fun getRestrictionStatus(@PathVariable merchantId: UUID): RestrictionStatusResponse {
        log.info { "Fetching restriction status for merchant: $merchantId" }

        val organization = organizationRepository.findById(merchantId)
            .orElseThrow { RecordNotFoundException("Organization not found: $merchantId") }

        return RestrictionStatusResponse(
            restricted = organization.restricted,
            reason = organization.restrictionReason
        )
    }

    @PutMapping("/{merchantId}/restriction")
    @PreAuthorize("hasAuthority('SCOPE_internal')")
    @Transactional
    @ResponseStatus(HttpStatus.OK)
    fun restrictMerchant(
        @PathVariable merchantId: UUID,
        @RequestBody request: RestrictMerchantRequest
    ): RestrictionStatusResponse {
        log.info { "Restricting merchant: $merchantId, reason: ${request.reason}" }

        val organization = organizationRepository.findById(merchantId)
            .orElseThrow { RecordNotFoundException("Organization not found: $merchantId") }

        organization.restricted = true
        organization.restrictedAt = Instant.now()
        organization.restrictionReason = request.reason
        organization.updatedAt = Instant.now()
        organizationRepository.save(organization)

        log.info { "Merchant $merchantId restricted successfully" }

        return RestrictionStatusResponse(
            restricted = true,
            reason = request.reason
        )
    }

    @DeleteMapping("/{merchantId}/restriction")
    @PreAuthorize("hasAuthority('SCOPE_internal')")
    @Transactional
    @ResponseStatus(HttpStatus.OK)
    fun liftRestriction(@PathVariable merchantId: UUID): RestrictionStatusResponse {
        log.info { "Lifting restriction for merchant: $merchantId" }

        val organization = organizationRepository.findById(merchantId)
            .orElseThrow { RecordNotFoundException("Organization not found: $merchantId") }

        organization.restricted = false
        organization.restrictedAt = null
        organization.restrictionReason = null
        organization.updatedAt = Instant.now()
        organizationRepository.save(organization)

        log.info { "Restriction lifted for merchant $merchantId" }

        return RestrictionStatusResponse(restricted = false, reason = null)
    }
}

data class RestrictionStatusResponse(
    val restricted: Boolean,
    val reason: String?
)

data class RestrictMerchantRequest(
    val reason: String
)
